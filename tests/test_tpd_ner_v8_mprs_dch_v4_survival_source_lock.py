...
from experiments import (
    freeze_tpd_ner_v4_survival_exact_source_lock as freezer,
)
from experiments import (
...
class FreezeV4SurvivalExactSourceLockTests(unittest.TestCase):
    def test_cli_has_exactly_three_actions_and_trainer_defaults(self) -> None:
    ...
    def test_payload_binds_data_sources_formal_stats_and_parent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
        ...
        self.assertEqual(
            first["survival_target_statistics"]["path"],
            str(
                trainer.DEFAULT_TARGET_STATISTICS_PATH.relative_to(
                    trainer.REPO_ROOT
                )
            ),
        )
    ...
    def test_plan_is_read_only_and_reports_all_external_bindings(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            dataset_dir = make_tiny_dataset(root)
            output = root / "survival-source-lock.json"
    ...
    def test_write_once_postverifies_exact_four_key_consumer(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            dataset_dir = make_tiny_dataset(root)
            output = root / "survival-source-lock.json"
    ...
    def test_verify_rejects_each_external_binding_tamper(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            dataset_dir = make_tiny_dataset(root)

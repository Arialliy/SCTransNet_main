...
from experiments.validate_tpd_clean_v4_2x_completion import (
...
SUMMARIZER = REPO_ROOT / "experiments/summarize_tpd_clean_v4_screen800.py"
VALIDATOR = (
    REPO_ROOT / "experiments/validate_tpd_clean_v4_2x_completion.py"
)
POSTPROCESS_LOCK = (
    REPO_ROOT / "experiments/tpd_clean_v4_2x_postprocess_source_lock.json"
)
TRAINING_LOCK = (
...
def test_runner_waits_for_exact_four_worker_bundles() -> None:
    ...
    for tag in ("full-s42", "cap-s42", "full-s3407", "cap-s3407"):
        assert f"sctransnet-tpd-clean-v4-2x-$v4_tag.service" in text
        assert tag in text
    for filename in (
        "metrics.jsonl",
        "summary.json",
    ...
    assert '[[ "$v4_metrics_count" -eq 800 ]]' in text
    assert "TPDCLEANV4_2X_COMPLETE variant=$v4_variant seed=$v4_seed" in text
    assert "TPDCLEANV4_2X_ABORT|Traceback|out of memory" in text
    assert "resume" in text
    assert "TPDCLEANV4_2X_FINALIZER_WAIT" in text


def test_runner_delegates_marker_last_publication_to_validator() -> None:
    text = RUNNER.read_text(encoding="utf-8")

    assert '"$v4_completion_validator" publish' in text
    assert '"$v4_completion_validator" verify' in text
    ...
    assert "train_tpd_clean_v4.py" not in text
    assert "evaluate_tpd_clean_v4_pd_fa.py" not in text
...
def test_launcher_preflight_precedes_any_persistent_start() -> None:
    ...
    preflight = text.index('if [[ "$v4_mode" == "--preflight" ]]')
    start = text.index('"$v4_systemd_run" --user')
    assert preflight < start
    assert "v4_verify_postprocess_sources" not in text
    assert "TPDCLEANV4_2X_POSTPROCESS_SOURCES_OK" in text
    assert "sctransnet-tpd-clean-v4-2x-screen800-finalizer.service" in text
    assert "/usr/bin/bash \"$v4_runner\"" in text
    assert "train_tpd_clean_v4.py" not in text
...
def test_launcher_preflight_does_not_start_finalizer() -> None:
    ...
    assert "TPDCLEANV4_2X_FINALIZER_PREFLIGHT_OK" in completed.stdout
...
def test_runner_wait_limit_is_nonpublishing_in_isolated_root(
    ...
    environment = {
        **os.environ,
        "TPDCLEANV4_2X_FINALIZER_RESULT_ROOT": str(candidate_root),
        "TPDCLEANV4_2X_FINALIZER_SYSTEMCTL": str(fake_systemctl),
        "TPDCLEANV4_2X_FINALIZER_SLEEP": "/bin/true",
        "TPDCLEANV4_2X_FINALIZER_POLL_SECONDS": "0",
        "TPDCLEANV4_2X_FINALIZER_MAX_POLLS": "1",
    }
    ...
    assert "TPDCLEANV4_2X_FINALIZER_WAIT poll=1" in completed.stdout
    assert not (
        candidate_root / "NUDT-SIRST/comparison/COMPLETE.sha256"
    ).exists()
    assert not (
        candidate_root
        / "NUDT-SIRST/comparison/tpd_clean_v4_screen800_comparison.json"
    ).exists()
    state = json.loads(

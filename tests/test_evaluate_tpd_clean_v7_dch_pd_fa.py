...
from experiments import evaluate_tpd_clean_v7_dch_pd_fa as evaluator


class TPDCleanV7DCHClosedIntervalEvaluatorTests(unittest.TestCase):
    ...
    def test_reuses_v6_closed_interval_core_without_metric_override(self) -> None:
        probabilities = [
        ...
        self.assertEqual(
            contract["determinism"], evaluator.DETERMINISM_SETTINGS
        )

    def test_configure_dch_inference_applies_exact_cpu_contract(
        self,
    ) -> None:
        original = self._torch_settings()
        try:
            observed = evaluator.configure_dch_inference("cpu")
            self.assertEqual(
                {
                    key: observed[key]
    ...
    def test_cuda_configuration_requires_exact_cublas_environment(
        self,
    ) -> None:
        original = self._torch_settings()
        try:
            with mock.patch.dict(
                os.environ, {"CUBLAS_WORKSPACE_CONFIG": ""}, clear=False
            ):
                with self.assertRaisesRegex(
                    RuntimeError, "CUBLAS_WORKSPACE_CONFIG"
                ):
                    evaluator.configure_dch_inference("cuda:0")
            with mock.patch.dict(
                os.environ,
                {
                ...
                observed = evaluator.configure_dch_inference("cuda:0")
            self.assertEqual(
    ...
    def test_main_binds_only_dch_builder_closed_core_and_wrapper_identity(
        self,
    ) -> None:
        ...
        try:
            with (
                ...
                mock.patch.object(
                    sys, "argv", [evaluator.__file__, "--device", "cpu"]
                ),
            ):
            ...
            self.assertIs(
                observed["builder"],
                evaluator.build_clean_v7_dch_model,
            )

...
from experiments import evaluate_tpd_clean_v4_pd_fa as evaluator


class TPDCleanV4ClosedIntervalSweepTests(unittest.TestCase):
    def test_appends_exact_one_plateau_and_empty_boundary(self) -> None:
        ...
        self.assertEqual(
            thresholds.count(evaluator.LAST_FLOAT32_BELOW_ONE), 1
        )
        self.assertEqual(
            thresholds.count(evaluator.UPPER_BOUNDARY_THRESHOLD), 1
        )
    ...
    def test_main_binds_v4_builder_variants_and_wrapper_source(self) -> None:
        self.assertEqual(
            evaluator.SUPPORTED_CLEAN_V4_VARIANTS,
            ("tpd_clean_v4_full", "tpd_clean_v4_sal_capacity"),
        )
        ...
        try:
            with mock.patch.object(
                evaluator.base, "main", side_effect=capture_bindings
            ) as base_main:
                evaluator.main()
            ...
            self.assertIs(observed["builder"], evaluator.build_clean_v4_model)

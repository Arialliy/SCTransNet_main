...
from experiments import train_tpd_ner_v4_qfg_v2_croa_exact as entry
...
class _TinyHaar(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.register_buffer("kernels", torch.zeros(1))


class _TinyQFGLevel(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.alpha = nn.Parameter(torch.zeros(()))
        self.haar = _TinyHaar()
        self.prior_projection = nn.Conv2d(1, 1, 1, bias=False)
        self.spatial_projection = nn.Sequential(
            nn.Conv2d(1, 1, 1, bias=False)
        )
        self.gate_out = nn.Conv2d(1, 1, 1, bias=False)
        with torch.no_grad():
            self.gate_out.weight.zero_()


class TinyQFGSurvivalModel(nn.Module):
    """Small executable graph with the formal 544+4+20 state topology."""
    ...
    def __init__(self) -> None:
        ...
        self.tpd_qfg = nn.Module()
        self.tpd_qfg.levels = nn.ModuleList(
            [_TinyQFGLevel() for _ in range(4)]
        )
        with torch.no_grad():
    ...
    def forward(self, images: torch.Tensor):
        ...
        for level in self.tpd_qfg.levels:
            scalar = scalar + 0.0 * level.gate_out.weight.sum()
...
def extension_provenance() -> dict:
    return {
        ...
        "preserved_new_state_key_count": 24,
        "new_module_prefixes": ["target_survival", "tpd_qfg"],
        "zero_init_prefixes": list(entry.QFG_ZERO_INIT_PREFIXES),
    }
...
class V4QFGV2CROAExactTrainerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.previous_threads = torch.get_num_threads()
        torch.set_num_threads(1)
        cls.args = parse(entry.QFG_ONLY_VARIANT)
        cls.model, cls.metadata = entry.build_selected_model(
            entry.QFG_ONLY_VARIANT,
            42,
        )
        ...
        cls.spec = entry.make_exact_run_spec(
            ...
            environment={"name": "cpu-qfg-fixture"},
        )
    ...
    def test_parser_freezes_c_d_axes_and_only_tss_weight_differs(self) -> None:
        control = self.args
        enabled = parse(entry.TSS_QFG_VARIANT)
        self.assertEqual(control.survival_weight, 0.0)
        self.assertEqual(enabled.survival_weight, 0.005)
        self.assertEqual(control.qfg_variant, entry.QFG_VARIANT)
        self.assertEqual(enabled.qfg_variant, entry.QFG_VARIANT)
        self.assertEqual(control.tss_variant, entry.TSS_CONTROL_VARIANT)
        self.assertEqual(enabled.tss_variant, entry.TSS_ON_VARIANT)
        self.assertEqual(
        ...
        self.assertEqual(
            control.run_tag,
            entry.FORMAL_QFG_ONLY_RUN_TAG,
        )
        self.assertEqual(
            enabled.run_tag,
            entry.FORMAL_TSS_QFG_RUN_TAG,
        )
        for trailing in (
            ["--seed", "7"],
            ["--base-lr", "0.001"],
            ...
            with self.subTest(trailing=trailing):
                with self.assertRaises((ValueError, SystemExit)):
                    parse(entry.QFG_ONLY_VARIANT, trailing)

    def test_builder_is_integrated_and_initialization_contract_is_exact(
        self,
    ) -> None:
        self.assertIs(
            type(self.model),
        ...
        self.assertEqual(len(state), 568)
        self.assertEqual(
        ...
        self.assertEqual(
            {
                name
                for name in state
                if name.startswith(entry.QFG_STATE_PREFIX)
            },
            set(entry.QFG_STATE_KEYS),
        )
        for name in entry.QFG_TERMINAL_STATE_KEYS:
            self.assertEqual(int(torch.count_nonzero(state[name])), 0)
        ...
        self.assertEqual(manifest["qfg_variant"], entry.QFG_VARIANT)
        self.assertEqual(manifest["state_key_count"], 568)
        self.assertEqual(manifest["total_parameters"], 10_870_228)

    def test_run_identity_is_arm_owned_and_rejects_cross_arm(self) -> None:
        identity = entry.require_qfg_run_identity(
            self.identity,
            label="fixture",
            expected_variant=entry.QFG_ONLY_VARIANT,
        )
        determinism = identity["training_contract"]["determinism"]
        self.assertEqual(
            determinism["qfg_run_identity_schema"],
            entry.RUN_IDENTITY_SCHEMA,
        )
        self.assertEqual(determinism["qfg_variant"], entry.QFG_VARIANT)
        self.assertEqual(
            determinism["tss_variant"],
            entry.TSS_CONTROL_VARIANT,
        )
        self.assertEqual(
            determinism["new_module_prefixes"],
            ["target_survival", "tpd_qfg"],
        )
        self.assertEqual(
            identity["training_contract"]["loss"]["survival"][
                "survival_weight"
            ],
            0.0,
        )
        with self.assertRaisesRegex(ValueError, "differs"):
            entry.require_qfg_run_identity(
                self.identity,
                label="cross-arm",
                expected_variant=entry.TSS_QFG_VARIANT,
            )
        ...
        with self.assertRaisesRegex(ValueError, "loss contract"):
            entry.require_qfg_run_identity(wrong, label="wrong weight")

    def test_qfg_only_loss_does_not_build_or_read_tss_path(self) -> None:
    ...
    def test_checkpoint_adapter_emits_explicit_qfg_tss_and_sources(
        self,
    ) -> None:
        state = {
        ...
        state.update(
            {name: torch.zeros(1) for name in entry.SURVIVAL_STATE_KEYS}
        )
        state.update(
            {name: torch.zeros(1) for name in entry.QFG_STATE_KEYS}
        )
        ...
        self.assertEqual(len(checkpoint["state_dict"]), 568)
        self.assertEqual(checkpoint["qfg_variant"], entry.QFG_VARIANT)
        self.assertEqual(
            checkpoint["tss_variant"],
            entry.TSS_CONTROL_VARIANT,
        )
        self.assertEqual(
            checkpoint["architecture_manifest"],
            self.metadata["architecture_manifest"],
        )
        self.assertEqual(
            checkpoint["exact_source_lock_sha256"],
            checkpoint["source_locks"][entry.SOURCE_LOCK_KEY],
        )
        self.assertEqual(
    ...
    def test_cli_qfg_only_runs_one_tiny_cpu_epoch_and_commits(self) -> None:
        args = parse(entry.QFG_ONLY_VARIANT)
        args.epochs = 1
        toy_model = TinyQFGSurvivalModel()
        self.assertEqual(len(toy_model.state_dict()), 568)
        self.assertEqual(
            {
                name
                for name in toy_model.state_dict()
                if name.startswith(entry.QFG_STATE_PREFIX)
            },
            set(entry.QFG_STATE_KEYS),
        )
        ...
        with tempfile.TemporaryDirectory() as directory_text:
            ...
            self.assertEqual(events[0]["qfg_variant"], entry.QFG_VARIANT)
            self.assertEqual(events[0]["train_survival_loss"], 0.0)

...
from experiments import evaluate_tpd_ner_v8_mprs_dch_v3_pd_fa as subject
...
def source_binding(training_sha256: str = "1" * 64) -> dict:
    return {
        ...
        "determinism_core": {
            "path": "/fixture/determinism-core.py",
            "relative_path": "experiments/determinism-core.py",
            "sha256": "6" * 64,
        },
    }
...
class V3EvaluatorContractTests(unittest.TestCase):
    def test_v3_owned_identity_and_five_budget_contract(self) -> None:
        self.assertIs(subject.exact, exact)
        self.assertEqual(
            subject.EVALUATION_SCHEMA,
            "sctransnet_tpd_ner_v8_mprs_dch_v3_pd_fa_v1",
        )
        self.assertEqual(
            subject.EVALUATION_SOURCE_BINDING_SCHEMA,
            (
                "sctransnet_tpd_ner_v8_mprs_dch_v3_"
                "evaluation_source_binding_v1"
            ),
        )
        self.assertEqual(subject.TRAINING_SCHEMA, exact.EXACT_SOURCE_LOCK_SCHEMA)
        self.assertEqual(
            subject.ACCEPTANCE_SCHEMA,
            subject.v3_freeze.ACCEPTANCE_SCHEMA,
        )
        self.assertEqual(
    ...
    def test_authoritative_builder_has_exact_v3_structure(self) -> None:
        model, metadata = subject.build_model(subject.VARIANT, 42)
        relay_keys = {
            name
            for name in model.state_dict()
            if name.startswith("tpd_ner.")
        }
        self.assertEqual(len(relay_keys), 19)
        self.assertEqual(
            {
                name
            ...
            set(subject.EXPECTED_DC_OFFSET_STATE_KEYS),
        )
        self.assertEqual(
            sum(
                parameter.numel()
        ...
        self.assertEqual(
            metadata["architecture_manifest"]["gate_dc_offset_count"],
            3,
        )
        del model
    ...
    def test_gate_contract_is_stable_under_copy(self) -> None:
        ...
        self.assertFalse(
            observed["v2_predecessor_absolute_gate_required"]
        )
        self.assertEqual(len(observed["all_required_components"]), 6)
        self.assertTrue(observed["tiny_pd_reported_not_independent_gate"])
        self.assertEqual(
            observed[
    ...
    def test_fixed_normalization_and_coverage_preserve_every_metric(
        self,
    ) -> None:
        payload, artifact = raw_sweep_fixture()
        fixed, budgets = subject._validate_raw_points_and_summaries(
        ...
        self.assertEqual(
            set(fixed),
            set(subject.FIXED_THRESHOLD_COVERAGE_FIELDS),
        )
        self.assertEqual(fixed["tiny_target_count"], 39)
        self.assertEqual(fixed["matched_tiny_target_count"], 39)
        self.assertEqual(fixed["tiny_pd"], 1.0)
        for name in (
            "niou",
            "pixel_precision",
            "pixel_recall",
            ...
            self.assertEqual(
                fixed[name],
                payload["fixed_threshold_0_5"][name],
            )

        coverage = subject._final_metric_coverage(fixed, budgets)
        self.assertTrue(coverage["all_required_metrics_present"])
        self.assertEqual(
            coverage["required_fixed_threshold_fields"],
            list(subject.FIXED_THRESHOLD_COVERAGE_FIELDS),
        )
        self.assertEqual(coverage["fixed_threshold_0_5"], fixed)
        subject._validate_final_metric_coverage(
            coverage,
            fixed=fixed,
            budgets=budgets,
        )

    def test_fixed_and_coverage_reject_each_missing_required_field(
        self,
    ) -> None:
        payload, artifact = raw_sweep_fixture()
        raw_fixed = payload["fixed_threshold_0_5"]
        fixed, budgets = subject._validate_raw_points_and_summaries(
            payload,
            artifact_audit=artifact,
        )
        coverage = subject._final_metric_coverage(fixed, budgets)
        for name in subject.FIXED_THRESHOLD_COVERAGE_FIELDS:
            with self.subTest(layer="normalize", field=name):
                missing = copy.deepcopy(raw_fixed)
                missing.pop(name)
                with self.assertRaises(ValueError):
                    subject._normalize_fixed(missing)
            with self.subTest(layer="coverage", field=name):
                missing = copy.deepcopy(coverage)
                missing["fixed_threshold_0_5"].pop(name)
        ...
        false_complete = copy.deepcopy(coverage)
        false_complete["all_required_metrics_present"] = False
        with self.assertRaisesRegex(ValueError, "final metric coverage"):
            subject._validate_final_metric_coverage(
                false_complete,
                fixed=fixed,
                budgets=budgets,
    ...
    def test_source_binding_rejects_stale_shared_metric_core(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            training = root / "v3-training-lock.json"
            acceptance = root / "v3-acceptance-lock.json"
            training_payload = {
            ...
            source_paths = (
                ...
                subject.BASE_EVALUATOR_PATH.resolve(),
                subject.CLOSED_INTERVAL_CORE_PATH.resolve(),
                subject.DETERMINISM_CORE_PATH.resolve(),
            )
            acceptance_sources = {
                str(path.relative_to(subject.REPO_ROOT)): (
                    subject._sha256_file(path)
                )
                for path in source_paths
            }
            acceptance_payload = {
                "schema": subject.ACCEPTANCE_SCHEMA,
                "lock_kind": "acceptance",
                "candidate_family": exact.CANDIDATE_FAMILY,
                "dataset": subject.DATASET,
                "variants": [subject.VARIANT],
                "required_control": subject.V1_CONTROL,
                "structural_predecessor": subject.V2_PREDECESSOR,
                "relay_off_retrained": False,
                "relay_version": exact.V3_RELAY_VERSION,
                "training_source_lock_sha256": (
                    subject._sha256_file(training)
                ),
                "performance_gate_contract": (
                    subject.performance_gate_contract()
                ),
                "source_count": len(acceptance_sources),
                "source_sha256": acceptance_sources,
                "policy": {"official_test_accessed": False},
            }
            ...
            self.assertEqual(
                ...
                subject._sha256_file(subject.BASE_EVALUATOR_PATH),
            )
            acceptance_payload["source_sha256"][
    ...
    def test_missing_default_manifests_are_explicitly_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            with (
                mock.patch.object(
    ...
    def test_checkpoint_sha_guard_rejects_change_during_torch_load(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as directory:
            run_dir = Path(directory)
            checkpoint = run_dir / "best.pth.tar"
            checkpoint.write_bytes(b"before-load")

            def mutate_during_load(*_args, **_kwargs) -> dict:
                checkpoint.write_bytes(b"after-load")
                return {}

            with (
                mock.patch.object(subject, "_load_json", return_value={}),
                mock.patch.object(
                    subject,
                    "_validate_metrics",
                    return_value=([], {}),
                ...
                self.assertRaisesRegex(
                    ValueError,
                    "checkpoint SHA during preflight load differs",
                ),
            ):
                subject.validate_run_artifacts(
                    run_dir,
                    "best.pth.tar",
                )
    ...
    def test_exact_checkpoint_adapter_is_accepted_end_to_end(self) -> None:
        ...
        manifest = {
            ...
            "relay_version": exact.V3_RELAY_VERSION,
            "relay_width": 8,
            "relay_state_key_count": 19,
            "relay_parameters": 11_291,
            "total_parameters": 10_854_446,
        ...
        checkpoint_state = {
            ...
            **{
                f"tpd_ner.fixture.{index}": (
                    torch.zeros(11_273)
                    if index == 0
                    else torch.tensor([float(index)])
                )
                for index in range(16)
            },
            **{
                name: torch.zeros(1)
                for name in subject.EXPECTED_DC_OFFSET_STATE_KEYS
            },
        }
        checkpoint = dict(
            exact.EvaluatorCheckpointAdapter(
                ...
                exact_runner.CompatibilityPayloadContext(
                    ...
                    exact_payload={
                        "model": {"state_dict": checkpoint_state},
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            wrong_offsets = copy.deepcopy(checkpoint)
            wrong_offsets["state_dict"][
                "tpd_ner.dc_offsets.5"
            ] = wrong_offsets["state_dict"].pop(
                "tpd_ner.dc_offsets.4"
            )
            wrong_offsets["state_dict_sha256"] = (
                exact_runner._state_content_sha256(
                    wrong_offsets["state_dict"],
                    "wrong offset fixture",
                )
            )
            torch.save(wrong_offsets, run_dir / "best.pth.tar")
            with (
                mock.patch.object(
                    subject,
                    "_current_evaluation_source_binding",

...
from experiments import validate_tpd_clean_v3_completion as validator


REPO_ROOT = Path(__file__).resolve().parents[1]
SUMMARY_HELPERS_PATH = (
    REPO_ROOT / "tests/test_summarize_tpd_clean_v3_screen800.py"
)
...
class CompletionFixture:
    def __init__(self, root: Path) -> None:
        ...
        self.summarizer_path = (
            self.experiments / "summarize_tpd_clean_v3_screen800.py"
        )
        shutil.copyfile(CANONICAL_SUMMARIZER_PATH, self.summarizer_path)
        self.validator_path = (
            self.experiments / "validate_tpd_clean_v3_completion.py"
        )
        ...
        self.postprocess_lock = (
            self.experiments / "tpd_clean_v3_postprocess_source_lock.json"
        )
        _write_source_lock(
            self.postprocess_lock,
            "sctransnet_tpd_clean_v3_postprocess_source_lock_v1",
            {
                "experiments/summarize_tpd_clean_v3_screen800.py": _sha256(
                    self.summarizer_path
                ),
                "experiments/validate_tpd_clean_v3_completion.py": _sha256(
                    self.validator_path
                ),
    ...
    def _write_launch_evidence(self) -> None:
        ...
        for index, (variant, seed) in enumerate(jobs, start=1):
            ...
            (
                ...
                "worker output\n"
                f"TPDCLEANV3_COMPLETE variant={variant} seed={seed} "
...
class TPDV3CompletionValidatorTests(unittest.TestCase):

#!/usr/bin/env python3
"""Audit TPD-Clean-v4 screen800 artifacts and evaluate protocol Gates A--E.
...
DEFAULT_CANDIDATE_ROOT = (
    REPO_ROOT / "experiments/results/tpd_clean_v4_screen800_2x5090_v1"
)
DEFAULT_FORMAL_REFERENCE_ROOT = (
    REPO_ROOT / "experiments/results/tpd_pe_formal800_4x5090_v1"
)
DEFAULT_SMOKE_ROOT = (
    REPO_ROOT / "experiments/results/tpd_clean_v4_preflight_v1"
)
DEFAULT_TRAINING_SOURCE_LOCK = (
    REPO_ROOT / "experiments/tpd_clean_v4_screen800_2x_source_lock.json"
)
...
PRIMARY_VARIANT = "tpd_clean_v4_full"
CONTROL_VARIANT = "tpd_clean_v4_sal_capacity"
SEEDS = (42, 3407)
...
EXPECTED_FAMILY = "spd_anchored_tpd_clean_v4_single_logit_kcs"
EXPECTED_EVALUATOR = REPO_ROOT / "experiments/evaluate_tpd_clean_v4_pd_fa.py"
PROTOCOL_PATH = REPO_ROOT / "experiments/TPD_CLEAN_V4_PROTOCOL.md"
...
def evaluate_engineering_gate(
    ...
    """Evaluate TPD_CLEAN_V4_PROTOCOL.md Gates A--E without defaults."""
    normalized = {
    ...
    return {
        ...
        "checks": checks,
        "protocol_section": "TPD_CLEAN_V4_PROTOCOL.md section 5",
    }
...
def _validate_model_metadata(
    ...
    expected = {
        ...
        "fusion_formula": (
            "K+S*tanh(saliency_scale+0.5*tanh(context_scale)*context_code)"
        ),
        "context_logit_limit": 0.5,
    ...
    expected["context_reference"] = (
        "contrast" if variant == PRIMARY_VARIANT else "capacity_control"
    )
    for key, value in expected.items():
...
def _load_and_strict_check_checkpoints(
    ...
    try:
        ...
        raise IncompleteArtifact(f"{label}: cannot import v4 builder: {exc}") from exc
    ...
    for role, (filename, checkpoint_role, epoch, expected_metrics) in specs.items():
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            model, metadata = build_clean_v4_model(variant, seed)
...
def validate_training_source_lock(path: Path) -> tuple[dict[str, Any], str]:
    payload = _load_json(path, "v4 training source lock")
    _require(
        payload.get("schema")
        == "sctransnet_tpd_clean_v4_screen800_2x_source_lock_v1",
        "v4 training source-lock schema differs",
    )
    _require(payload.get("variants") == list(VARIANTS), "source-lock variants")
    _require(payload.get("model_seeds") == list(SEEDS), "source-lock seeds")
    entries = payload.get("source_sha256")
    _require(isinstance(entries, dict) and entries, "source-lock entries missing")
    ...
    evaluator_relative = "experiments/evaluate_tpd_clean_v4_pd_fa.py"
    _require(
        entries.get(evaluator_relative) == sha256_file(EXPECTED_EVALUATOR),
        "source lock does not bind the v4 evaluator",
    )
    return payload, sha256_file(path)


def validate_smoke_reports(smoke_root: Path) -> dict[str, Any]:
    manifest_path = smoke_root / "SMOKE_REPORTS.sha256"
    _regular(manifest_path, "smoke hash manifest")
    expected_names = {"cpu_all.json", "gpu2_full.json", "gpu3_capacity.json"}
    bound: dict[str, str] = {}
    for line in manifest_path.read_text(encoding="utf-8").splitlines():
        digest, separator, path_text = line.partition("  ")
        _sha(digest, "smoke manifest digest")
        _require(separator == "  ", "invalid smoke manifest line")
        path = Path(path_text)
        _require(path.parent.resolve() == smoke_root.resolve(), "smoke path escaped")
        _regular(path, "smoke report")
        _require(sha256_file(path) == digest, f"smoke hash differs: {path.name}")
        bound[path.name] = digest
    _require(set(bound) == expected_names, "smoke report set differs")

    reports = {
        name: _load_json(smoke_root / name, f"smoke/{name}")
        for name in expected_names
    }
    expected_variants = {
        "cpu_all.json": set(VARIANTS),
        "gpu2_full.json": {PRIMARY_VARIANT},
        "gpu3_capacity.json": {CONTROL_VARIANT},
    }
    for name, report in reports.items():
        _require(
            report.get("schema") == "sctransnet_tpd_clean_v4_smoke_v1"
            and report.get("status") == "complete"
            and report.get("paired_initialization") is True,
            f"smoke/{name}: report contract",
        )
        if name == "cpu_all.json":
            _require(
                report.get("device") == "cpu"
                and report.get("device_name") == "cpu",
                "CPU smoke device",
            )
        else:
            _require(
        ...
        for entry in variants:
            _require(
                ...
                f"smoke/{name}/{entry.get('variant')}: invariant failed",
            )
            for key in (
                "scale_gradient_l1",
                "phase_gradient_l1",
                ...
                _require(
                    ...
                    f"smoke/{name}/{entry.get('variant')}: {key}",
                )
    return {
...
def build_report(
    ...
    try:
        smoke = validate_smoke_reports(smoke_root)
    except IncompleteArtifact as exc:
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# TPD-Clean-v4 screen800 comparison",
        "",
        f"Status: **{status}**",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit TPD-Clean-v4 screen800 and evaluate Gates A--E"
    )
    parser.add_argument(

...
v3_formal_root="${TPDCLEANV3_2X_FINALIZER_FORMAL_ROOT:-$v3_repo/experiments/results/tpd_pe_formal800_4x5090_v1}"
v3_v2_root="${TPDCLEANV3_2X_FINALIZER_V2_ROOT:-$v3_repo/experiments/results/tpd_clean_screen800_4x5090_v1}"
v3_reference_miou_root="${TPDCLEANV3_2X_FINALIZER_REFERENCE_MIOU_ROOT:-$v3_v2_root/frozen_reference_miou_runs}"
...
v3_sleep="${TPDCLEANV3_2X_FINALIZER_SLEEP:-sleep}"
v3_poll_seconds="${TPDCLEANV3_2X_FINALIZER_POLL_SECONDS:-60}"
...
if [[ ! "$v3_poll_seconds" =~ ^([0-9]+([.][0-9]+)?|[.][0-9]+)$ ]]; then
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=invalid_poll_seconds value=$v3_poll_seconds" >&2
    exit 2
fi
...
[[ -x "$v3_python" ]] || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=python_not_executable path=$v3_python" >&2
    exit 1
}
[[ -f "$v3_summarizer" && ! -L "$v3_summarizer" ]] || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=missing_summarizer path=$v3_summarizer" >&2
    exit 1
}
[[ -f "$v3_completion_validator" && ! -L "$v3_completion_validator" ]] || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=missing_completion_validator path=$v3_completion_validator" >&2
    exit 1
}
[[ -f "$v3_postprocess_lock" && ! -L "$v3_postprocess_lock" ]] || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=missing_postprocess_lock path=$v3_postprocess_lock" >&2
    exit 1
}
command -v "$v3_systemctl" >/dev/null 2>&1 || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=missing_systemctl command=$v3_systemctl" >&2
    exit 1
}
command -v "$v3_sleep" >/dev/null 2>&1 || {
    echo "TPDCLEANV3_2X_FINALIZER_ABORT reason=missing_sleep command=$v3_sleep" >&2
    exit 1
}
...
payload = {
    "schema": "sctransnet_tpd_clean_v3_2x_screen800_finalizer_state_v1",
...
exec 9>"$v3_lock_file"
if ! flock -n 9; then
    echo "TPDCLEANV3_2X_FINALIZER_ALREADY_RUNNING lock=$v3_lock_file"
    exit 0
fi
...
if [[ -e "$v3_marker" || -L "$v3_marker" ]]; then
    if v3_verify_complete_marker; then
        ...
        echo "TPDCLEANV3_2X_FINALIZER_COMPLETE reused=true marker=$v3_marker"
        exit 0
    fi
    v3_write_state \
        failed \
        "An existing Clean-v3 completion marker failed validation."
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=invalid_existing_marker marker=$v3_marker" >&2
    exit 1
fi
...
if not (
    ...
    raise SystemExit(1)
completion_prefix = f"TPDCLEANV3_2X_COMPLETE variant={variant} seed={seed} "
if not any(
...
while true; do
    ...
    if [[ -n "$v3_failure" ]]; then
        ...
        echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=$v3_failure" >&2
        exit 1
    fi
    ...
    if (( v3_any_active != 1 )); then
        v3_write_state \
            ...
            "$v3_snapshot_file"
        rm -f "$v3_snapshot_file"
        echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=no_active_or_complete_worker" >&2
        exit 1
    fi
    ...
    rm -f "$v3_snapshot_file"
    v3_snapshot_file="-"
    echo "TPDCLEANV3_2X_FINALIZER_WAITING poll_seconds=$v3_poll_seconds"
...
then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=artifact_validation" >&2
    exit 1
fi
...
if ! v3_verify_postprocess_sources; then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=postprocess_source_drift_before_aggregation" >&2
    exit 1
fi
...
then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=summarizer_exit staging=$v3_staging_dir" >&2
    exit 1
fi
...
if [[ "$v3_summarizer_sha_before" != "$v3_summarizer_sha_after" ]]; then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=summarizer_changed" >&2
    exit 1
fi
if ! v3_verify_postprocess_sources; then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=postprocess_source_drift_during_aggregation" >&2
    exit 1
fi
...
if ! v3_publish_complete_bundle "$v3_staging_dir"; then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=completion_publish staging=$v3_staging_dir" >&2
    exit 1
fi
if ! v3_verify_complete_marker; then
    v3_write_state \
        ...
        "$v3_snapshot_file"
    rm -f "$v3_snapshot_file"
    echo "TPDCLEANV3_2X_FINALIZER_FAILED reason=completion_verify marker=$v3_marker" >&2
    exit 1
fi
...
v3_write_state \
    ...
    "$v3_snapshot_file"
rm -f "$v3_snapshot_file"
echo "TPDCLEANV3_2X_FINALIZER_COMPLETE reused=false marker=$v3_marker"

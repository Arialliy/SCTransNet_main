#!/usr/bin/env python3
"""Run the TPD-Clean-v3 Pd--Fa sweep on the closed probability interval.

The frozen v1 evaluator samples thresholds strictly below one.  A sigmoid
output can nevertheless contain values rounded to exactly 1.0 in FP32.  In
that case no sampled threshold can express the valid empty-prediction
operating point, and strict Fa budgets are reported as unavailable.

This recovery wrapper leaves the frozen evaluator unchanged and appends both
the last representable FP32 threshold below one and the probability-domain
boundary threshold 1.0.  The penultimate point isolates scores saturated to
exactly one.  ValidationMetrics applies ``prediction > threshold``, so the
boundary point is deterministically the empty-prediction point (Pd=0 and
Fa=0); it cannot improve detection quality.
"""
...
def adaptive_thresholds_closed_interval(
    ...
    """Return the frozen grid plus saturated-score and empty endpoints."""
...
def main() -> None:
    ...
    # The frozen evaluator records ``__file__`` in both its invocation and
    # evaluator digest.  Bind those fields to this behavior-changing wrapper.

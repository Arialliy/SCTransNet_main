...
v3_runner="${TPDCLEANV3_FINALIZER_RUNNER:-$v3_repo/experiments/run_tpd_clean_v3_screen800_finalizer.sh}"
...
v3_systemd_run="${TPDCLEANV3_FINALIZER_SYSTEMD_RUN:-systemd-run}"
...
cd "$v3_repo"
[[ -x "$v3_runner" ]] || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=runner_not_executable path=$v3_runner" >&2
    exit 1
}
[[ -f "$v3_summarizer" && ! -L "$v3_summarizer" ]] || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=missing_summarizer path=$v3_summarizer" >&2
    exit 1
}
[[ -f "$v3_completion_validator" && ! -L "$v3_completion_validator" ]] || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=missing_completion_validator path=$v3_completion_validator" >&2
    exit 1
}
[[ -x "$v3_python" ]] || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=python_not_executable path=$v3_python" >&2
    exit 1
}
[[ -f "$v3_postprocess_lock" && ! -L "$v3_postprocess_lock" ]] || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=missing_postprocess_lock path=$v3_postprocess_lock" >&2
    exit 1
}
command -v "$v3_systemctl" >/dev/null 2>&1 || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=missing_systemctl command=$v3_systemctl" >&2
    exit 1
}
command -v "$v3_systemd_run" >/dev/null 2>&1 || {
    echo "TPDCLEANV3_FINALIZER_LAUNCH_ABORT reason=missing_systemd_run command=$v3_systemd_run" >&2
    exit 1
}
bash -n "$v3_runner"
"$v3_python" - \
...
echo "TPDCLEANV3_FINALIZER_PREFLIGHT_OK poll_seconds=60 unit=$v3_unit"
...
case "$v3_active_state" in
    active|activating|deactivating|reloading)
        echo "TPDCLEANV3_FINALIZER_UNIT_REUSED state=$v3_active_state unit=$v3_unit"
        exit 0
        ;;
esac
...
echo "TPDCLEANV3_FINALIZER_UNIT_STARTED unit=$v3_unit"

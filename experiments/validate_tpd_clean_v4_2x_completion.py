#!/usr/bin/env python3
"""Publish and verify the hash-bound TPD-Clean-v4 screen800 bundle.

The canonical summarizer is the sole producer of the scientific report.  This
module only supplies the completion transaction around that report:

* re-run the canonical audit before publication;
* bind the exact 62-file evidence set in ``completion_inputs.json``;
...
DEFAULT_SUMMARIZER = (
    REPO_ROOT / "experiments/summarize_tpd_clean_v4_screen800.py"
)
...
MANIFEST_SCHEMA = "sctransnet_tpd_clean_v4_completion_inputs_v1"
...
EXPECTED_GPU_ASSIGNMENTS = {
    ("tpd_clean_v4_full", 42): GPU2_UUID,
    ("tpd_clean_v4_sal_capacity", 42): GPU3_UUID,
    ("tpd_clean_v4_full", 3407): GPU3_UUID,
    ("tpd_clean_v4_sal_capacity", 3407): GPU2_UUID,
}
...
LOCK_SPECS = {
    ...
    "postprocess": (
        "experiments/tpd_clean_v4_2x_postprocess_source_lock.json",
...
def _load_all_locks(
    *,
    ...
    v3_path = locks["v3"][0]
    if (
        training.get("frozen_v3_source_lock_sha256")
        != _sha256_file(v3_path)
    ):
        _fail("v4 training source lock does not bind the current v3 lock")
    if (
...
def _load_canonical_summarizer(
    *,
    ...
    module_name = (
        "_tpd_clean_v4_canonical_summary_"
        + summarizer_sha[:16]
        + "_"
        + str(os.getpid())
    )
    ...
    if module.JSON_OUTPUT_NAME != "tpd_clean_v4_screen800_comparison.json":
        _fail("canonical summarizer JSON output name changed")
    if module.MARKDOWN_OUTPUT_NAME != "tpd_clean_v4_screen800_comparison.md":
        _fail("canonical summarizer Markdown output name changed")
    if module.SCHEMA != "sctransnet_tpd_clean_v4_screen800_comparison_v1":
...
def _expected_input_descriptors(
    *,
    ...
    training_lock = _relative_regular_file(
        repo, LOCK_SPECS["training"][0], "v4 training source lock"
    )
...
def publish_completion(
    *,
    ...
    training_lock = _relative_regular_file(
        config["repo"], LOCK_SPECS["training"][0], "v4 training source lock"
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Publish or verify the exact 62-input TPD-Clean-v4 "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"TPDCLEANV4_2X_COMPLETION_INVALID reason={exc}",
            file=sys.stderr,
        )
        return 1
    print(
        "TPDCLEANV4_2X_COMPLETION_OK"

...
case "$v3_variant:$v3_seed:$v3_gpu_uuid" in
    tpd_clean_v3_full:42:GPU-4a0f4ab5-9d4e-20d9-4e7a-515e2d4e0562) ;;
    tpd_clean_v3_sal_capacity:42:GPU-8d68eb9e-49d3-67f6-f715-6ef2ac4975c3) ;;
    tpd_clean_v3_full:3407:GPU-8d68eb9e-49d3-67f6-f715-6ef2ac4975c3) ;;
    tpd_clean_v3_sal_capacity:3407:GPU-4a0f4ab5-9d4e-20d9-4e7a-515e2d4e0562) ;;
    *)
        echo "TPDCLEANV3_2X_ABORT reason=invalid_job_mapping variant=$v3_variant seed=$v3_seed gpu_uuid=$v3_gpu_uuid" >&2
        exit 2
        ;;
esac
...
if ! flock -n 9; then
    echo "TPDCLEANV3_2X_ABORT reason=lock_held variant=$v3_variant seed=$v3_seed" >&2
    exit 1
fi
...
print(
    f"TPDCLEANV3_2X_SOURCES_OK locks={len(seen)} checked_entries={file_count}",
    flush=True,
)
PY
}
...
v3_verify_data() {
    ...
    if [[ "$v3_actual" != "$v3_training_data_sha256" ]]; then
        echo "TPDCLEANV3_2X_ABORT reason=training_data_drift expected=$v3_training_data_sha256 actual=$v3_actual" >&2
        return 1
    fi
}
...
v3_verify_gpu() {
    ...
    if [[ "$v3_name" != "NVIDIA GeForce RTX 5090" ]]; then
        echo "TPDCLEANV3_2X_ABORT reason=unexpected_gpu gpu_uuid=$v3_gpu_uuid name=$v3_name" >&2
        return 1
    fi
    if (( v3_free < 7500 )); then
        echo "TPDCLEANV3_2X_ABORT reason=insufficient_memory gpu_uuid=$v3_gpu_uuid free_mib=$v3_free" >&2
        return 1
    fi
...
if [[ -e "$v3_run_dir" || -L "$v3_run_dir" ]]; then
    echo "TPDCLEANV3_2X_ABORT reason=run_path_not_fresh variant=$v3_variant seed=$v3_seed path=$v3_run_dir" >&2
    exit 1
fi

v3_write_launch_manifest

echo "TPDCLEANV3_2X_START variant=$v3_variant seed=$v3_seed gpu_uuid=$v3_gpu_uuid run_dir=$v3_run_dir"
...
if [[ "$(v3_sha256 "$v3_source_lock")" != "$v3_source_lock_sha256" ]]; then
    echo "TPDCLEANV3_2X_ABORT reason=source_lock_changed_during_run" >&2
    exit 1
fi
echo "TPDCLEANV3_2X_COMPLETE variant=$v3_variant seed=$v3_seed gpu_uuid=$v3_gpu_uuid epochs=800"

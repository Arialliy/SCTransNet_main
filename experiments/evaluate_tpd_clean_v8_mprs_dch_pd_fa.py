#!/usr/bin/env python3
"""V8-MPRS-DCH wrapper over the closed-interval Pd/Fa evaluator."""
...
def configure_v8_inference(device: str) -> Dict[str, Any]:
    """Apply the same deterministic inference settings as V7-DCH."""

    if device == "cuda:0" and os.environ.get(
        "CUBLAS_WORKSPACE_CONFIG"
    ) != CUBLAS_WORKSPACE_CONFIG:
        raise RuntimeError(
            "CUDA V8 evaluator requires "
    ...
    if observed != DETERMINISM_SETTINGS:
        raise RuntimeError(
            f"V8 evaluator deterministic settings differ: {observed!r}"
        )
    return {
        **observed,
        "device": device,
...
def evaluator_contract() -> Dict[str, Any]:
    """Return the V8 wrapper identity and inherited metric semantics."""

    return {
        "candidate_family": "tpd_clean_v8_mprs_dch",
        "variants": list(SUPPORTED_CLEAN_V8_MPRS_DCH_VARIANTS),
...
def main() -> None:
    if SUPPORTED_CLEAN_V8_MPRS_DCH_VARIANTS != (
        "tpd_clean_v8_mprs_dch_full",
        "tpd_clean_v8_mprs_dch_capacity",
    ):
        raise RuntimeError("Unexpected TPD-Clean V8-MPRS-DCH variants")
    configure_v8_inference(requested_device())

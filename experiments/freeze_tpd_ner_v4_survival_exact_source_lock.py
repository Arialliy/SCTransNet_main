#!/usr/bin/env python3
"""Plan, write once, or verify the V4 target-survival exact source lock.
...
closure.  The freezer itself is deliberately outside the numerical runtime
source set.
"""
...
LOCK_KIND = "training"
PLAN_SCHEMA = "sctransnet_tpd_ner_v4_survival_exact_source_lock_plan_v1"
ACTION_SCHEMA = "sctransnet_tpd_ner_v4_survival_exact_source_lock_action_v1"
...
def runtime_source_sha256(
    *,
    ...
    """Hash the trainer's complete, ordered-independent runtime closure."""
    ...
    if not declared:
        raise ValueError("Survival runtime source set is empty")
    ...
    for index, source in enumerate(declared):
        ...
        if relative in records:
            raise ValueError(f"duplicate Survival runtime source: {relative}")
...
def survival_target_statistics_binding() -> dict[str, Any]:
    """Independently bind the validated target-statistics artifact."""

    resolved, relative = _repo_relative_regular_file(
        trainer.DEFAULT_TARGET_STATISTICS_PATH,
        repo_root=REPO_ROOT,
        label="Survival target statistics",
    )
    digest = _sha256_regular_file(resolved, "Survival target statistics")
    statistics = trainer.load_survival_target_statistics(resolved)
    _require_equal(
        "Survival target-statistics SHA",
        statistics.get("sha256"),
        digest,
    )
    _require_equal(
        "Survival target-statistics schema",
...
def parent_checkpoint_binding() -> dict[str, Any]:
    """Independently bind the immutable V4 best-mIoU extension parent."""

    resolved, relative = _repo_relative_regular_file(
        trainer.PARENT_CHECKPOINT_PATH,
        repo_root=REPO_ROOT,
        label="Survival parent checkpoint",
    )
    digest = _sha256_regular_file(resolved, "Survival parent checkpoint")
    _require_equal(
        "Survival parent checkpoint SHA",
        digest,
...
def build_source_lock_payload(
    *,
    ...
    """Build the deterministic Survival lock payload without writing it."""

    root = Path(repo_root).resolve()
    if root != trainer.REPO_ROOT.resolve():
        raise ValueError(
            "Survival source lock must be built against trainer.REPO_ROOT"
        )
    variants = trainer.supported_candidate_variants()
    _require_equal(
        "Survival source-lock variant matrix",
        variants,
...
def _load_json_object(path: Path) -> dict[str, Any]:
    value = _regular_file(path, "Survival source lock")
    try:
        payload = json.loads(value.read_text(encoding="utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(
            f"invalid Survival source-lock JSON: {value}"
        ) from exc
    if not isinstance(payload, dict):
        raise ValueError("Survival source lock must contain one JSON object")
    return payload
...
def _require_new_output(path: Path) -> Path:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(
            "refusing to overwrite existing Survival source lock: "
            f"{output}"
        )
...
def verify_source_lock(
    ...
    *,
    ...
    _require_equal("Survival source-lock payload", observed, expected)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plan, exclusively write, or verify the V4 target-survival "
            "exact training source lock"
        )
    )

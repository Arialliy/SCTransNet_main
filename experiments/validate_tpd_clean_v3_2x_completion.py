...
DEFAULT_POSTPROCESS_LOCK = (
    REPO_ROOT / "experiments/tpd_clean_v3_2x_postprocess_source_lock.json"
)
...
EXPECTED_GPU_ASSIGNMENTS = {
    ("tpd_clean_v3_full", 42): GPU2_UUID,
    ("tpd_clean_v3_sal_capacity", 42): GPU3_UUID,
    ("tpd_clean_v3_full", 3407): GPU3_UUID,
    ("tpd_clean_v3_sal_capacity", 3407): GPU2_UUID,
}
...
def _load_canonical_summarizer(
    *,
    ...
    expected_relative = "experiments/summarize_tpd_clean_v3_screen800_2x5090.py"
    ...
    postprocess = _load_source_lock(
        ...
        repo=repo,
        expected_schema="sctransnet_tpd_clean_v3_2x_postprocess_source_lock_v1",
...
def _validate_launch_and_log(
    *,
    ...
    expected_gpu_uuid = EXPECTED_GPU_ASSIGNMENTS[(variant, seed)]
    if gpu_uuid != expected_gpu_uuid:
        _fail(
            f"launch manifest {variant}/seed={seed}: gpu_uuid={gpu_uuid!r}, "
            f"expected counterbalanced assignment {expected_gpu_uuid!r}"
        )
    ...
    pattern = re.compile(
        rf"^TPDCLEANV3_2X_COMPLETE variant={re.escape(variant)} "
...
def _expected_input_descriptors(
    *,
    ...
    if gpu_uuids != EXPECTED_GPU_UUIDS:
        _fail(
            "launch manifests do not bind exactly the registered GPU2/GPU3 set: "
            f"{gpu_uuids}"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"TPDCLEANV3_2X_COMPLETION_INVALID reason={exc}", file=sys.stderr)
        return 1
    print(
        "TPDCLEANV3_2X_COMPLETION_OK"

#!/usr/bin/env python3
"""Train the isolated TPD-Clean-v4 stable KCS fusion candidates.

The split, augmentation, optimizer, schedule, losses, metrics, and checkpoint
selection remain inherited from ``train_tpd_pilot``.  Only shallow
``embeddings_1/2`` are replaced.
"""
...
from model.tpd_clean_v4 import (  # noqa: E402
    PRIMARY_CLEAN_V4_VARIANT,
    SUPPORTED_CLEAN_V4_VARIANTS,
    clean_v4_variant_spec,
    parameter_count,
    replace_shallow_embeddings_clean_v4,
)


def build_clean_v4_model(
    variant: str, seed: int
) -> Tuple[SCTransNet, Dict[str, Any]]:
    """Build a paired SCTransNet whose only change is V4 KCS embeddings."""
    ...
    replacements = replace_shallow_embeddings_clean_v4(model, variant)
    ...
    metadata = {
        ...
        "primary_candidate": variant == PRIMARY_CLEAN_V4_VARIANT,
...
def main() -> None:
    ...
    # v1/v2/v3 runners remain byte-for-byte unchanged.
    base.SUPPORTED_VARIANTS = SUPPORTED_CLEAN_V4_VARIANTS

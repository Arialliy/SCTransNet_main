#!/usr/bin/env python3
"""Exact epoch-boundary trainer for the sole V2 five-node NER candidate.

The numerical loop is the verified V1/V8 exact loop, cloned with V2-owned
builders, identities, source locks, checkpoint schemas, and pre-restore
guards.  Exact resume is accepted only for the same V2 relay-on trajectory.
V1 relay-on/off checkpoints and every other variant are rejected before any
model, optimizer, scaler, RNG, or DataLoader-generator state is restored.
"""
...
ENTRY_SCHEMA = "sctransnet_tpd_ner_v8_mprs_dch_v2_exact_entry_v1"
EXACT_SOURCE_LOCK_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v2_exact_source_lock_v1"
)
ARCHITECTURE_MANIFEST_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v2_exact_architecture_manifest_v1"
)
CHECKPOINT_IDENTITY_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v2_exact_checkpoint_identity_v1"
)
CHECKPOINT_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v2_exact_checkpoint_v1"
)
COMPLETION_SUMMARY_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v2_completion_summary_v1"
)
SOURCE_LOCK_KEY = "tpd_ner_v8_mprs_dch_v2_exact_source_lock"
RUN_ID_PREFIX = "tpd-ner-v8-mprs-dch-v2-exact:"
FORMAL_RUN_TAG = "formal800_exact_v2_seed42"

TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON = (
    "tpd_ner_v8_mprs_dch_v2_full_relay_on"
)
V1_RELAY_OFF_REFERENCE = "tpd_ner_v8_mprs_dch_full_relay_off"
V1_RELAY_ON_VARIANT = "tpd_ner_v8_mprs_dch_full_relay_on"
...
DEFAULT_EXACT_SOURCE_LOCK_PATH = (
    REPO_ROOT / "experiments/tpd_ner_v8_mprs_dch_v2_exact_source_lock.json"
)
DEFAULT_OUTPUT_ROOT = (
    REPO_ROOT / "experiments/results/tpd_ner_v8_mprs_dch_v2_exact_v1"
)
...
# No lock file is created here.  The future lock must cover the V2-owned
# sources plus every inherited source whose code executes in the cloned loop.
RUNTIME_SOURCE_PATHS = _ordered_unique_paths(
    (
        REPO_ROOT / "experiments/train_tpd_ner_v8_mprs_dch_v2_exact.py",
        REPO_ROOT / "experiments/train_tpd_ner_v8_mprs_dch_v2.py",
        REPO_ROOT / "model/tpd_ner_v8_mprs_dch_v2.py",
        REPO_ROOT / "experiments/TPD_NER_V8_MPRS_DCH_V2_PROTOCOL.md",
...
def supported_candidate_variants() -> tuple[str, ...]:
    ...
    if tuple(variants) != FALLBACK_CANDIDATE_VARIANTS:
        raise ValueError("ordinary and exact V2 candidate matrices differ")
...
def candidate_contract(candidate_variant: str) -> dict[str, Any]:
    ...
    if candidate_variant != TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON:
        raise ValueError(
            f"unsupported V2 exact variant {candidate_variant!r}; "
...
def _validate_formal_args(args: argparse.Namespace) -> None:
    ...
    if observed != expected:
        raise ValueError(
            f"formal V2 exact arguments differ: "
    ...
    if allow_cpu_smoke:
        ...
        raise ValueError("formal V2 exact training requires --device=cuda:0")
...
def _replace_option_value(
    ...
    for index, argument in enumerate(arguments):
        ...
        if argument.startswith(f"{option}="):
            ...
            return
    raise ValueError(f"V2 exact entry requires {option}")


def _emit_v2_help() -> None:
    output = io.StringIO()
    try:
        with contextlib.redirect_stdout(output):
            v1_exact.parse_args(
                ["--variant", V1_RELAY_ON_VARIANT, "--help"]
            )
    ...
    help_text = help_text.replace(
        ...
        "Exact-resume V2 RMS-balanced centered-gate five-node NER training",
    )
    v1_choices = (
...
def _mapped_v1_arguments(
    ...
    for argument in arguments:
        if any(
            ...
            raise ValueError("V2 relay identity is fixed by the sole variant")
    ...
    for index, argument in enumerate(mapped):
        ...
        if argument.startswith("--variant="):
            ...
            break
    if selected != TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON:
        raise ValueError(
            "V2 exact entry accepts only "
            f"--variant={TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON}"
        )
    _replace_option_value(mapped, "--variant", V1_RELAY_ON_VARIANT)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    if any(value in ("-h", "--help") for value in arguments):
        _emit_v2_help()
    mapped = _mapped_v1_arguments(arguments)
    args = v1_exact.parse_args(mapped)
    args.variant = TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON
...
def require_v2_run_identity(
    identity: Any,
    *,
    label: str,
    expected_variant: str | None = None,
) -> dict[str, Any]:
    """Require a same-version, same-variant V2 exact trajectory identity."""

    if not isinstance(identity, Mapping):
        raise ValueError(f"{label} has no V2 exact run identity")
    ...
    if entry_schema in {
        ...
        raise ValueError(f"{label} is a V1/V8 trajectory, not V2")
    if variant in {V1_RELAY_OFF_REFERENCE, V1_RELAY_ON_VARIANT}:
        raise ValueError(f"{label} uses a V1 relay variant")
    if entry_schema != ENTRY_SCHEMA:
        raise ValueError(f"{label} entry schema is not V2")
    if variant != TPD_NER_V8_MPRS_DCH_V2_FULL_RELAY_ON:
        raise ValueError(f"{label} variant is not the sole V2 candidate")
    ...
    if not isinstance(run_id, str) or not run_id.startswith(RUN_ID_PREFIX):
        raise ValueError(f"{label} run_id is not V2")
    forbidden_lock_keys = {
    ...
    if (
        ...
        raise ValueError(f"{label} source-lock identity is not V2")
    ...
    for name, expected in required_determinism.items():
        if determinism.get(name) != expected:
            raise ValueError(f"{label} V2 determinism field {name} differs")
...
_require_v2_run_identity = require_v2_run_identity
...
def _existing_training_contract(
    ...
    protocol = load_json_mapping(
        ...
        "existing V2 exact protocol",
    )
    if protocol.get("schema") != ENTRY_SCHEMA:
        raise ValueError("existing protocol is not a V2 exact run")
    identity = require_v2_run_identity(
    ...
    if not isinstance(training, Mapping):
        raise ValueError("existing V2 protocol has no training contract")
    required = (
    ...
    if missing:
        raise ValueError(f"existing V2 training contract lacks fields: {missing}")
...
def initialization_plan(
    ...
    if args.exact_resume:
        ...
        if not isinstance(initial_rng, Mapping):
            raise ValueError("existing V2 initial_rng is not a mapping")
        if not isinstance(selection_policy, Mapping):
            raise ValueError("existing V2 selection_policy is not a mapping")
        return InitializationPlan(
    ...
    raise RuntimeError("V2 exact entry requires --fresh or --exact-resume")
...
def _architecture_manifest(
    ...
    return {
        ...
        "exact_resume_scope": "same_v2_relay_on_variant_only",
...
def build_selected_model(
    ...
    *,
    ...
    if seed != TRAINING_SEED:
        raise ValueError("V2 exact builder requires seed=42")
    if eps != FORMAL_EPS:
        raise ValueError(f"V2 exact builder requires eps={FORMAL_EPS}")
    ordinary = _ordinary_module()
    builder = getattr(
        ordinary,
    ...
    if not callable(builder):
        raise TypeError("ordinary V2 trainer does not expose its builder")
    model, raw_metadata = builder(variant, seed)
    if type(model) is not TPDNERV8MPRSDCHV2SCTransNet:
        raise TypeError("V2 exact builder requires the exact V2 model class")
    if (
        model.relay_enabled is not True
        ...
        raise ValueError("V2 exact model identity differs")
    relay_keys = [
        name for name in model.state_dict() if name.startswith("tpd_ner.")
    ]
    if len(relay_keys) != 16:
        raise ValueError(f"V2 relay state-key count differs: {len(relay_keys)}")
    if v2_relay_parameter_count(model) != PRODUCTION_V2_RELAY_PARAMETERS:
        raise ValueError("V2 exact relay parameter count differs")
    if sum(parameter.numel() for parameter in model.parameters()) != (
        PRODUCTION_V2_RELAY_ON_PARAMETERS
    ):
        raise ValueError("V2 exact total parameter count differs")
    if not isinstance(raw_metadata, Mapping):
        raise TypeError("ordinary V2 builder metadata is not a mapping")
    metadata = copy.deepcopy(dict(raw_metadata))
    if (
        metadata.get("variant") != variant
        or metadata.get("relay_enabled") is not True
        or metadata.get("relay_version") != "v2_rms_centered_arctangent"
        or metadata.get("parent_variant") != PRIMARY_CLEAN_V8_MPRS_DCH_VARIANT
    ):
        raise ValueError("ordinary V2 builder metadata differs")
    ...
    if (
        ...
        raise ValueError("ordinary V2 architecture_id is invalid")
    metadata["ordinary_builder_architecture_id"] = ordinary_architecture_id
...
def environment_contract(device: torch.device) -> dict[str, Any]:
    ...
    if physical_index not in PHYSICAL_GPU_UUIDS:
        raise RuntimeError(
            "V2 physical GPU index must identify registered GPU 2 or 3"
        )
...
def source_lock_contract(
    ...
    payload = load_json_mapping(path, "V2 exact source lock")
    if payload.get("schema") != EXACT_SOURCE_LOCK_SCHEMA:
        raise ValueError("V2 exact source-lock schema mismatch")
    if tuple(payload.get("variants", ())) != supported_candidate_variants():
        raise ValueError("V2 exact source-lock variant matrix mismatch")
    if payload.get("formal_contract") != formal_contract():
        raise ValueError("V2 exact source-lock formal contract mismatch")
    if payload.get("training_data_sha256") != training_data_sha256:
        raise ValueError("training data differs from the V2 source lock")
    locked = payload.get("source_sha256")
    if not isinstance(locked, Mapping):
        raise ValueError("V2 exact source lock has no source mapping")
    required = {
    ...
    if missing:
        raise ValueError(f"V2 exact source lock omits runtime sources: {missing}")
    for relative, expected in locked.items():
        if not isinstance(relative, str) or not relative:
            raise ValueError("V2 source lock has an invalid path")
        runtime = (REPO_ROOT / relative).resolve()
        try:
            canonical = str(runtime.relative_to(REPO_ROOT))
        except ValueError as exc:
            raise ValueError("V2 source path escapes repository") from exc
        if canonical != relative:
            raise ValueError("V2 source path is not canonical")
        if file_sha256(runtime) != expected:
            raise ValueError(f"V2 source lock differs for {relative}")
    return {
...
def make_exact_run_spec(
    ...
    *,
    ...
    if (
        ...
        or manifest.get("relay_version") != "v2_rms_centered_arctangent"
        or manifest.get("relay_enabled") is not True
        or manifest.get("relay_width") != RELAY_WIDTH
    ):
        raise ValueError("V2 exact architecture manifest differs")
...
def _require_complete_validation_metrics(
    ...
    if missing:
        raise ValueError(f"V2 validation metrics lack fields: {missing}")
    return {
...
def require_evaluator_checkpoint_payload(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("V2 exact evaluator checkpoint is not a mapping")
    ...
    if missing:
        raise ValueError(f"V2 evaluator checkpoint lacks fields: {missing}")
    if value["schema"] != CHECKPOINT_SCHEMA:
        raise ValueError("V2 evaluator checkpoint schema differs")
    identity = require_v2_run_identity(
        value["run_identity"],
    ...
    for name, expected in expected_top_level.items():
        if value.get(name) != expected:
            raise ValueError(f"V2 evaluator checkpoint {name} differs")
    epoch = value["epoch"]
    if (
        ...
        raise ValueError("V2 evaluator checkpoint epoch is invalid")
    ...
    if (
        ...
        raise ValueError("V2 evaluator checkpoint role is invalid")
    for name in ("state_dict", "optimizer", "scaler"):
        if not isinstance(value[name], Mapping) or (
            name == "state_dict" and not value[name]
        ):
            raise ValueError(f"V2 evaluator checkpoint {name} is invalid")
    ...
    if (
        ...
        raise ValueError("V2 evaluator checkpoint scheduler differs")
    ...
    for name, metric in metrics.items():
        if (
            ...
            raise ValueError(f"V2 evaluator metric {name} is invalid")
    ...
    if (
        ...
        or metadata.get("required_control") != V1_RELAY_OFF_REFERENCE
        or metadata.get("relay_off_retrained") is not False
    ):
        raise ValueError("V2 evaluator model metadata differs")
    ...
    for name, digest in split_hashes.items():
        if (
            ...
            raise ValueError("V2 evaluator split hash is invalid")
        int(digest, 16)
    ...
    if (
        ...
        raise ValueError("V2 evaluator checkpoint identity differs")
    return value
...
class TPDNERV8V2ExactRunner(v1_exact.TPDNERV8ExactRunner):
    """Exact runner that validates V2 identity before restoring any state."""
    ...
    def _require_v8_active_journal(self) -> None:
        ...
        if not isinstance(payload.get("optimizer"), Mapping):
            raise ValueError("active V2 journal has no optimizer state")


DCHExactRunner = TPDNERV8V2ExactRunner
...
def protocol_payload(
    ...
    *,
    ...
    payload["exact_resume_policy"] = {
        "same_version": "same_v2_relay_on_epoch_boundary_only",
        "v1_relay_off": "forbidden",
        "v1_relay_on": "forbidden",
...
def _load_complete_events(
    ...
    if len(events) != epochs or [
        ...
        raise RuntimeError("V2 exact metrics are not a contiguous history")
    for event in events:
...
def completion_summary(
    ...
    *,
    ...
    payload.update(
        {
            ...
            "candidate_family": "tpd_ner_v8_mprs_dch_v2",
    ...
    protocol = load_json_mapping(
        directory / "protocol.json",
        "completed V2 exact protocol",
    )
    if protocol.get("schema") != ENTRY_SCHEMA:
        raise ValueError("completed protocol is not V2")
    payload["run_identity"] = require_v2_run_identity(
        protocol.get("run_identity"),
    ...
    for name, value in metrics.items():
        if (
            ...
            raise FloatingPointError(
                f"non-finite V2 validation metric {name!r}: {value!r}"
            )

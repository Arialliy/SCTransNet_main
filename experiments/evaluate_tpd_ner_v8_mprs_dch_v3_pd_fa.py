#!/usr/bin/env python3
"""Formal closed-interval Pd/Fa evaluator for the sole V3 relay-on model."""
...
from experiments import train_tpd_ner_v8_mprs_dch_v3_exact as exact  # noqa: E402
...
EVALUATION_SCHEMA = "sctransnet_tpd_ner_v8_mprs_dch_v3_pd_fa_v1"
EVALUATION_SOURCE_BINDING_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v3_evaluation_source_binding_v1"
)
FINAL_METRIC_COVERAGE_SCHEMA = (
    "sctransnet_tpd_ner_v8_mprs_dch_v3_final_metric_coverage_v1"
)
TRAINING_SCHEMA = v3_freeze.TRAINING_SCHEMA
ACCEPTANCE_SCHEMA = v3_freeze.ACCEPTANCE_SCHEMA
DATASET = str(exact.formal_contract()["dataset"])
VARIANT = exact.TPD_NER_V8_MPRS_DCH_V3_FULL_RELAY_ON
V1_CONTROL = v3_freeze.V1_CONTROL
V2_PREDECESSOR = v3_freeze.V2_PREDECESSOR
TRAINING_SEED = 42
SPLIT_SEED = 20260722
EXPECTED_EPOCHS = 800
...
FA_BUDGETS = tuple(v3_freeze.FA_BUDGETS)
BUDGET_KEYS = tuple(f"{budget:.10g}" for budget in FA_BUDGETS)
EXTRA_THRESHOLDS = (0.001, 0.005, 0.995, 0.999, 0.9995, 0.9999)
DEFAULT_TRAINING_LOCK = v3_freeze.DEFAULT_TRAINING_LOCK
DEFAULT_ACCEPTANCE_LOCK = v3_freeze.DEFAULT_ACCEPTANCE_LOCK
...
ISOLATED_MODULE_NAME = "_sctransnet_tpd_ner_v8_mprs_dch_v3_pd_fa"
...
FIXED_THRESHOLD_COVERAGE_FIELDS = (
    "threshold",
    "pd",
    "fa",
    "miou",
...
performance_gate_contract = v3_freeze.performance_gate_contract
...
def _current_evaluation_source_binding(
    *,
    ...
    """Validate and bind the executable V3 evaluator to frozen manifests."""
    ...
    acceptance_lock = Path(
        ...
        if acceptance_lock_path is None
        else acceptance_lock_path
    ).resolve()
    ...
    deterministic_module_file = (
        None
        if deterministic_module is None
        else getattr(deterministic_module, "__file__", None)
    )
    if deterministic_module_file is None:
        raise ValueError("deterministic inference core has no source file")
    _require_equal(
        "deterministic inference callable source",
        Path(deterministic_module_file).resolve(),
...
def verify_frozen_manifests(
    *,
    ...
    """Read-only public manifest verification helper."""

    return _current_evaluation_source_binding(
        training_lock_path=training_lock_path,
        acceptance_lock_path=acceptance_lock_path,
    )
...
def evaluator_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "determinism_core_sha256": source_binding[
            "determinism_core"
        ]["sha256"],
        ...
        "paired_gate_status": "requires_v3_postprocess_aggregate",
...
def _validate_v3_structure(
    model: Any,
    metadata: Mapping[str, Any],
) -> None:
    state = model.state_dict()
    relay_keys = {
        name for name in state if name.startswith("tpd_ner.")
    }
    _require_equal(
        "V3 model relay state-key count",
        len(relay_keys),
        EXPECTED_RELAY_STATE_KEY_COUNT,
    )
    _require_equal(
        "V3 model DC-offset state keys",
        {
            name
            for name in relay_keys
            if name.startswith("tpd_ner.dc_offsets.")
        },
        set(EXPECTED_DC_OFFSET_STATE_KEYS),
    )
    relay_parameters = sum(
        parameter.numel()
        for name, parameter in model.named_parameters()
        if name.startswith("tpd_ner.")
    )
    _require_equal(
        "V3 model relay parameter count",
        relay_parameters,
        EXPECTED_RELAY_PARAMETERS,
    )
    _require_equal(
        "V3 model total parameter count",
        sum(parameter.numel() for parameter in model.parameters()),
        EXPECTED_TOTAL_PARAMETERS,
    )
    manifest = _require_mapping(
        "V3 model architecture manifest",
    ...
    for name, expected in {
        ...
        _require_equal(
            f"V3 model architecture manifest {name}",
            manifest.get(name),
            expected,
        )


def _validate_v3_checkpoint_structure(
    ...
    _require_equal(
        "checkpoint relay state-key count",
        len(relay_keys),
        EXPECTED_RELAY_STATE_KEY_COUNT,
    )
    _require_equal(
        "checkpoint DC-offset state keys",
        {
            name
            for name in relay_keys
            if name.startswith("tpd_ner.dc_offsets.")
    ...
    relay_parameter_count = 0
    for name in relay_keys:
        tensor = state[name]
        if not isinstance(tensor, torch.Tensor):
            raise ValueError(f"checkpoint relay state {name} is not a tensor")
        relay_parameter_count += tensor.numel()
    _require_equal(
        "checkpoint relay parameter count",
    ...
    for name, expected in {
        ...
        _require_equal(
            f"checkpoint architecture manifest {name}",
            manifest.get(name),
            expected,
        )
    for name, expected in {
        ...
        _require_equal(
            f"checkpoint model metadata {name}",
            metadata.get(name),
            expected,
        )
...
def build_model(variant: str, seed: int):
    if variant != VARIANT:
        raise ValueError("V3 evaluator accepts only the relay-on candidate")
    if type(seed) is not int or seed != TRAINING_SEED:
        raise ValueError("V3 evaluator accepts only seed=42")
    model, metadata = exact.build_selected_model(variant, seed)
    metadata = _require_mapping("V3 builder metadata", metadata)
    _validate_v3_structure(model, metadata)
...
def _validate_run_identity(
    ...
    value = exact.require_v3_run_identity(
        identity,
    ...
    if type(physical_index) is not int or str(physical_index) not in (
        ...
        raise ValueError("formal V3 trajectory must use physical GPU 2 or 3")
...
def _validate_metrics(path: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if any(not line.strip() for line in raw_lines):
        raise ValueError("V3 metrics must not contain blank rows")
    ...
    if (
        ...
        raise ValueError("V3 metrics must be a contiguous 1..800 history")
    ...
    try:
        ...
        raise ValueError(f"V3 metrics selection history differs: {exc}") from exc
...
def validate_run_artifacts(
    ...
    """Validate one complete exact V3 run before model state is consumed."""

    run_dir = Path(run_dir).resolve()
    if not run_dir.is_dir() or run_dir.is_symlink():
        raise NotADirectoryError(run_dir)
    if checkpoint_name not in CHECKPOINT_ROLES:
        raise ValueError("V3 evaluator accepts only best or best_miou")
    ...
    if not isinstance(checkpoint, Mapping):
        raise ValueError("V3 compatibility checkpoint must be an object")
    ...
    _validate_v3_checkpoint_structure(checkpoint)
    ...
    for name, expected in {
        "parent_variant": "tpd_clean_v8_mprs_dch_full",
        "enabled": True,
        "version": exact.V3_RELAY_VERSION,
        "width": 8,
    ...
    _require_equal(
        "comparison structural predecessor",
        comparison.get("structural_predecessor"),
        V2_PREDECESSOR,
    )
    ...
    if not all(isinstance(record, Mapping) for record in model_records.values()):
        raise ValueError("V3 model metadata views must be objects")
...
def validate_formal_arguments(
    ...
    if args.overwrite:
        raise ValueError("V3 formal evaluator forbids --overwrite")
    if args.checkpoint not in CHECKPOINT_ROLES:
        raise ValueError("V3 evaluator accepts only best or best_miou")
    if args.device not in ("cpu", "cuda:0"):
        raise ValueError("V3 evaluator device must be cpu or cuda:0")
    if args.expected_epochs not in (None, EXPECTED_EPOCHS):
        raise ValueError("V3 evaluator requires expected_epochs=800")
    ...
    if args.match_radius not in (None, 3.0):
        raise ValueError("V3 evaluator match_radius must be omitted or 3.0")
    if args.tiny_area not in (None, 9):
        raise ValueError("V3 evaluator tiny_area must be omitted or 9")
    return args
...
def _validate_standard_integrity(audit: Mapping[str, Any]) -> None:
    ...
    if any(checks[name] is not True for name in REQUIRED_INTEGRITY_CHECKS):
        raise ValueError("V3 evaluator integrity checks are incomplete")
...
def _validate_raw_points_and_summaries(
    ...
    *,
    ...
    if not isinstance(raw_points, list) or not raw_points:
        raise ValueError("V3 sweep raw points are missing")
    points = [
        _validate_raw_point(
            point,
            index=index,
    ...
    if thresholds != sorted(thresholds) or len(thresholds) != len(set(thresholds)):
        raise ValueError("V3 sweep raw thresholds must be unique and sorted")
    ...
    if any(
        ...
        raise ValueError("V3 sweep raw threshold set is incomplete")
    _require_equal(
    ...
    if len(fixed_matches) != 1:
        raise ValueError("V3 sweep must contain exactly one threshold=0.5 point")
...
def _normalize_fixed(point: Any) -> dict[str, Any]:
    ...
    tiny_target_count = value.get("tiny_target_count")
    matched_tiny = value.get("matched_tiny_target_count")
    ...
    if type(tiny_target_count) is not int or tiny_target_count != 39:
        raise ValueError("fixed tiny target count differs")
    if (
        type(matched_tiny) is not int
        or not 0 <= matched_tiny <= tiny_target_count
    ):
        raise ValueError("fixed matched tiny target count differs")
    normalized = {
    ...
    _require_equal(
        "fixed tiny-Pd/count",
        normalized["tiny_pd"],
        matched_tiny / tiny_target_count,
    )
    for name in (
        "pd",
        "fa",
    ...
    _require_equal(
        "fixed normalized field registry",
        set(result),
        set(FIXED_THRESHOLD_COVERAGE_FIELDS),
    )
    return result
...
def _final_metric_coverage(
    ...
    normalized_fixed = _normalize_fixed(fixed)
    _require_equal(
        "coverage budget keys",
        set(budgets),
        set(BUDGET_KEYS),
    )
    fixed_coverage = {
        name: normalized_fixed[name]
        for name in FIXED_THRESHOLD_COVERAGE_FIELDS
    }
    all_required_metrics_present = (
        set(fixed_coverage) == set(FIXED_THRESHOLD_COVERAGE_FIELDS)
        and set(budgets) == set(BUDGET_KEYS)
    )
    if not all_required_metrics_present:
        raise ValueError("final metric coverage is incomplete")
    return {
        ...
        "fixed_threshold_0_5": fixed_coverage,
        "required_fa_budget_keys": list(BUDGET_KEYS),
        "pd_at_fa_budget": copy.deepcopy(dict(budgets)),
        "all_required_metrics_present": all_required_metrics_present,
    }


def _validate_final_metric_coverage(
    coverage: Any,
    *,
    fixed: Mapping[str, Any],
    budgets: Mapping[str, Any],
) -> None:
    observed = _require_mapping("final_metric_coverage", coverage)
    _require_canonical_json_equal(
...
def _absolute_gate(
    ...
    return {
        ...
        "paired_v3_on_vs_v1_off_gate_status": (
            "requires_same_role_aggregate"
        ),
        "paired_v3_on_vs_v2_on_gate_status": (
            "requires_same_role_aggregate"
        ),
...
def _validate_closed_interval(payload: Mapping[str, Any]) -> None:
    ...
    if not isinstance(points, list):
        raise ValueError("V3 sweep points are missing")
    by_threshold = {
    ...
    if (
        ...
        raise ValueError("V3 sweep endpoint points are missing")
...
def finalize_evaluation_output(
    ...
    if checkpoint_name not in CHECKPOINT_ROLES:
        raise ValueError("V3 evaluation checkpoint filename differs")
...
def validate_output_identity(
    ...
    *,
    ...
    _require_equal(
        ...
        payload.get("structural_predecessor"),
        V2_PREDECESSOR,
    )
    ...
    if (
        ...
        raise ValueError("V3 output run/checkpoint paths differ")
    _require_equal(
    ...
    _validate_final_metric_coverage(
    ...
    if (
        ...
        raise ValueError("V3 evaluator invocation identity differs")
    ...
    if not parsed_run.is_absolute() or parsed_run != run_dir:
        raise ValueError("V3 evaluator parsed run directory differs")
    ...
    try:
        ...
        try:
            ...
            raise FileExistsError(
                f"refusing to replace existing V3 sweep: {path}"
            ) from exc
    finally:
...
def main() -> None:
    ...
    if "-h" not in argv and "--help" not in argv:
        ...
        # Refuse an unfrozen production evaluation before configuring a
        # device or reading any run/checkpoint artifact.
        _current_evaluation_source_binding()
